      ...
      ({ className, code, language = 'plaintext', theme = 'dark-plus', disableCopy = false }: CodeBlockProps) => {
        ...
        useEffect(() => {
          if (language && !isSpecialLang(language) && !(language in bundledLanguages)) {
            logger.warn(`Lenguaje '${language}' no soportado`);
          }

          logger.trace(`Lenguaje = ${language}`);
        ...
        return (
          <div className={classNames('relative group text-left', className)}>
            ...
            >
              ...
              <button
                className="ml-2 flex items-center bg-accent-500 p-[6px] justify-center rounded-md transition-theme"
                title={isVisible ? "Ocultar código" : "Mostrar código"}
                onClick={() => setIsVisible(!isVisible)}
              >
                <div className={isVisible ? "i-ph:eye-slash-duotone" : "i-ph:eye-duotone"}></div>
              </button>
            </div>
            {isVisible && <div dangerouslySetInnerHTML={{ __html: html ?? '' }}></div>}
          </div>
        );
      },
    );
